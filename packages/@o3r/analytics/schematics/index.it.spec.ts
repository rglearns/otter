...
describe('new otter application with analytics', () => {
  ...
  test('should add analytics to existing application', () => {
    execSync(`${packageManagerCmd.add} @o3r/analytics@${o3rVersion}`, execAppOptions);
    execSync(`${packageManagerCmd.exec} ng add @o3r/analytics@${o3rVersion} --skip-confirmation --defaults=true --force --verbose`, execAppOptions);

    execSync(`${packageManagerCmd.exec} ng g @o3r/core:component --defaults=true test-component --use-otter-analytics=false`, execAppOptions);
    execSync(`${packageManagerCmd.exec} ng g @o3r/analytics:add-analytics --defaults=true --path="src/components/test-component/container/test-component-cont.component.ts"`, execAppOptions);
