...
describe('new otter application with configuration', () => {
  ...
  test('should add configuration to existing application', () => {
    execSync(`${packageManagerCmd.add} @o3r/configuration@${o3rVersion}`, execAppOptions);
    execSync(`${packageManagerCmd.exec} ng add @o3r/configuration@${o3rVersion} --verbose`, execAppOptions);

    execSync(`${packageManagerCmd.exec} ng g @o3r/core:component --defaults=true test-component --use-otter-config=false`, execAppOptions);
    execSync(`${packageManagerCmd.exec} ng g @o3r/configuration:add-config --defaults=true --path="src/components/test-component/container/test-component-cont.component.ts"`, execAppOptions);
