...
describe('new otter application with styling', () => {
  ...
  test('should add styling to existing application', () => {
    execSync(`${packageManagerCmd.add} @o3r/styling@${o3rVersion}`, execAppOptions);
    execSync(`${packageManagerCmd.exec} ng add @o3r/styling@${o3rVersion} --skip-confirmation --defaults=true --force --verbose --enable-cms`, execAppOptions);

    execSync(`${packageManagerCmd.exec} ng g @o3r/core:component --defaults=true test-component --use-otter-theming=false`, execAppOptions);
    execSync(`${packageManagerCmd.exec} ng g @o3r/styling:add-theming --defaults=true --path="src/components/test-component/presenter/test-component-pres.style.scss"`, execAppOptions);
